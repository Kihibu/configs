...
local colors = require("theme.onelight.colors")
...
function M.setup(config)
  ...
  theme.base = {
    ...
    CursorLineNr = { fg = c.dark5, bg = c.bg_highlight }, -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    MatchParen = { fg = c.red, bg = c.bg_highlight, style = "bold" }, -- The character under the cursor or just before it, if it is a paired bracket, and its match. |pi_paren.txt|
    ...
    IncSearch = { bg = c.red1, fg = c.light }, -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    ...
    Todo = { bg = c.yellow, fg = c.fg }, -- (preferred) anything that needs extra attention; mostly the keywords TODO FIXME and XXX
    qfLineNr = { fg = c.dark5 },
