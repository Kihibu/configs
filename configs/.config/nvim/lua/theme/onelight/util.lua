local hsluv = require("theme.onelight.hsluv")
...
function util.debug(colors)
  colors = colors or require("onelight.colors")
  -- Dump unused colors
...
function util.onColorScheme()
  if vim.g.colors_name ~= "onelight" then
    vim.cmd([[autocmd! onelight]])
    vim.cmd([[augroup! onelight]])
  end
end
...
function util.autocmds(config)
  vim.cmd([[augroup onelight]])
  vim.cmd([[  autocmd!]])
  vim.cmd([[  autocmd ColorScheme * lua require("theme.onelight.util").onColorScheme()]])
  if config.dev then
    vim.cmd([[  autocmd BufWritePost */lua/onelight/** nested colorscheme onelight]])
  end
